...
const connectionString = `postgres://postgres:BADmommydale@localhost/DALE_Gradebook`;
...
router.get('/', function(req, res){
    ...
    res.sendFile(path.join(__dirname, 'DALE_Gradebook.html'));
});
...
function populateGradebook(data){
    ...
    let tableElm = document .getElementById("gradebook"); // Get the gradebook table element
        data.forEach(function(assignment){ // for each row of data we're passed in 
            ...
            columns.name = document.createElement("td"); // The first column's table data will be teh name
            columns.name.appendChild(
                // Conatenate the full name:  "last_name, first_name"
